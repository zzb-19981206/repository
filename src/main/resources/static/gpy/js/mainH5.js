...
function openDev(){
	...
	setTimeout(function(){
		...
		//识别语言
		jsonObj = {FuncName:'ocrGetLanguageName'};
		sendWsMessage(jsonObj);
		
		jsonObj = {FuncName:'StartSignDevice'};
		sendWsMessage(jsonObj);
...
function Capture(){
	...
	var filepath = document.getElementById("saveText").value  + time.getYear() + time.getMonth ()+ time.getDate() + time.getDate ()+ time.getTime()+".jpg";
...
{
	var filepath = document.getElementById("HeadPath").value;
	var jsonObj = {FuncName:'idcardrfidReadIDCardEx',argument:{HeadPath:filepath}};
	sendWsMessage(jsonObj);
}
function showHead(){
	m_idcardBase64 = true;
...
//读取银行卡
function ReadBankCard(sort){
	
	var jsonObj = {FuncName:'ReadBankCard',argument:{sort:sort}};
	sendWsMessage(jsonObj);
	
}

//读取磁条卡
function ReadMagneticCard(sort){
	
	var jsonObj = {FuncName:'ReadMagneticCard',argument:{sort:sort}};
	sendWsMessage(jsonObj);
}
...
function CaptureBase64Ex()
{
	...
	var jsonObj = {FuncName:'CaptureEncodeBase64',argument:{nIndex:Index}};
	sendWsMessage(jsonObj);
}
//删除文件
function DeleteFile(){
...
function InitFaceSDK(){
	var jsonObj = {FuncName:'InitFaceSDK',argument:{}};
	sendWsMessage(jsonObj);
}
//反初始化人脸识别模块
function UinitFaceSDK(){
	
	var jsonObj = {FuncName:'UinitFaceSDK',argument:{}};
	sendWsMessage(jsonObj);
}


//打开人脸视频
function OpenFaceCamera(index){
	
	var jsonObj = {FuncName:'OpenFaceCamera',argument:{devIndex:index,subtype:0,width:0,height:0}};
	sendWsMessage(jsonObj);
	
}
//设置比对图片
 function SetComparePic(){
            	
	var url = document.getElementById("imgOne").value;
	
    var jsonObj = {FuncName:'SetHeadByFile',argument:{path:url}};
	sendWsMessage(jsonObj); 	
...
function comparePic(){
	...
	var jsonObj = {FuncName:'MatchFace',argument:{path:"D:\\123.jpg"}};
	sendWsMessage(jsonObj); 
	
}

//图片base64比对
function SetComparePicBase64(){
	
	var url = document.getElementById("imgBase64").value;
	var jsonObj = {FuncName:'SetHeadByBaseStr',argument:{bsStr:url}};
	sendWsMessage(jsonObj);
...
function coderRecBarcode()
{
	...
	var jsonObj = {FuncName:'coderRecognizeBarcode',argument:{strBarcodePath:BarcodePath,ileft:parseInt(w/2),itop:0,iright:parseInt(w),ibottom:parseInt(h/2)}};

	sendWsMessage(jsonObj);
	}
	
}
function pdf17RecBarcode()
{
	var BarcodePath = document.getElementById("OCR_Pathtext").value;
	var img = new Image();
	...
	var jsonObj = {FuncName:'coderRecognizeBarcodePDF417',argument:{strBarcodePath:BarcodePath,ileft:parseInt(w/2),itop:0,iright:parseInt(w),ibottom:parseInt(h/2)}};

	sendWsMessage(jsonObj);
	}
	
}
function ZbarRecBarcode()
{
	var BarcodePath = document.getElementById("OCR_Pathtext").value;
	...
	w = img.width;
	h = img.height;
	var jsonObj = {FuncName:'zbRecognizeBarcode',argument:{strBarcodePath:BarcodePath,ileft:0,itop:0,iright:parseInt(w/2),ibottom:parseInt(h/2)}};
...
function RecToFile(type)
{
	...
	//CamSDKOCX.CreateDirectory(strFolder);
	ocrType = type;
	
	var jsonObj = {FuncName:'ocrAddImageFileToOCRList',argument:{strImagePath:OCR_Pathtext.value}};
	sendWsMessage(jsonObj);
...
function GetSign()
{
	var jsonObj = {FuncName:'CmOutputImageBase64'};
	sendWsMessage(jsonObj);
}
